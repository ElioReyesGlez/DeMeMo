...
public class CustomLineView extends View {
    ...
    private Runnable animator = new Runnable() {
        @Override public void run() {
            ...
            for (ArrayList<CustomLineView.Dot> data : drawDotLists) {
                for (CustomLineView.Dot dot : data) {
                    dot.update();
                    if (!dot.isAtRest()) {
                        needNewFrame = true;
                    }
        ...
        if (dataLists != null && !dataLists.isEmpty()) {
            ...
            for (int k = 0; k < dataLists.size(); k++) {
                ...
                for (int i = 0; i < dataLists.get(k).size(); i++) {
                    ...
                    if (i > drawDotSize - 1) {
                        drawDotLists.get(k).add(new CustomLineView.Dot(x, 0, x, y, dataLists.get(k).get(i), k));
                    } else {
                        drawDotLists.get(k)
    ...
    @Override protected void onDraw(Canvas canvas) {
        ...
        for (int k = 0; k < drawDotLists.size(); k++) {
            ...
            for (CustomLineView.Dot d : drawDotLists.get(k)) {
    ...
    private void drawDots(Canvas canvas) {
        ...
        if (drawDotLists != null && !drawDotLists.isEmpty()) {
            for (int k = 0; k < drawDotLists.size(); k++) {
                ...
                for (CustomLineView.Dot dot : drawDotLists.get(k)) {
    ...
    private CustomLineView.Dot findPointAt(int x, int y) {
    ...
    private CustomLineView.Dot getCurrentPointDot(int day) {
        if (drawDotLists.isEmpty()) {
            return null;
        }
    ...
    class Dot {
        ...
        CustomLineView.Dot setTargetData(int targetX, float targetY, float data, int linenumber) {
